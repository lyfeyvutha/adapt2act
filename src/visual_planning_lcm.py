...
def rollout(cfg):
    ...
    if cfg.plan_with_probadap:
        from utils import AnimateLCMProbAdaptation
        guidance = AnimateLCMProbAdaptation(device, domain=cfg.domain, use_suboptimal=cfg.use_suboptimal)
        animatediff_pipe = guidance.pipe
        avdc_trainer = guidance.avdc_trainer
    else:
        from utils import AnimateLCM
        guidance = AnimateLCM(device)
        animatediff_pipe = guidance.pipe
    ...
    for rollout_idx in range(cfg.plan_num_rollouts):
        ...
        # rendered = Image.fromarray(env.render(**render_kwargs))
